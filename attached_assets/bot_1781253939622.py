...
# Memory settings
MEMORY_KEEP_LAST = 200       # messages kept per user in DB
MEMORY_CONTEXT_TURNS = 12    # messages passed to Claude as conversation history
...
async def remember(user_id: int, role: str, content: str):
    """Store a message in long-term memory. role: 'user' or 'pet' or 'event'."""
    if not content:
        return
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO conversations (user_id, role, content, ts) VALUES (?,?,?,?)",
            (user_id, role, content[:1000], datetime.utcnow().isoformat()),
        )
        # Trim: keep only the most recent MEMORY_KEEP_LAST rows per user
        await db.execute(
            """DELETE FROM conversations WHERE user_id=? AND id NOT IN (
                   SELECT id FROM conversations WHERE user_id=?
                   ORDER BY ts DESC LIMIT ?
               )""",
            (user_id, user_id, MEMORY_KEEP_LAST),
        )
        await db.commit()


async def recall(user_id: int, limit: int = MEMORY_CONTEXT_TURNS) -> list[dict]:
    """Fetch the last N memory rows, oldest first."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT role, content, ts FROM conversations WHERE user_id=? ORDER BY ts DESC LIMIT ?",
            (user_id, limit),
        ) as cur:
            rows = await cur.fetchall()
...
def memory_to_messages(memory: list[dict], current_text: str) -> list[dict]:
    """
    Convert memory rows into Anthropic messages format.
    'event' rows are folded into the next user message as bracketed context.
    Ensures alternating roles by merging consecutive same-role messages.
    """
    msgs = []
    pending_events = []
    ...
    for row in memory:
        ...
        if role == "event":
            pending_events.append(content)
            continue
        api_role = "user" if role == "user" else "assistant"
        text = content
        if api_role == "user" and pending_events:
            text = "[" + "; ".join(pending_events) + "]\n" + text
            pending_events = []
    ...
    # Current user message (with any unattached events)
    current = current_text
    if pending_events:
        current = "[" + "; ".join(pending_events) + "]\n" + current
    ...
    return msgs if msgs else [{"role": "user", "content": current_text}]
...
async def parse_text_intent(text: str) -> dict:
    ...
    msg = await claude.messages.create(
        ...
        system=(
            ...
            "— extract any context mentioned (после обеда/завтрака/ужина/сна/спорта/прогулки/натощак/перед сном). "
            "Empty string if none. "
            "If user wants to set baseline (моя норма 9.5, базовый 10): "
            "{\"intent\":\"baseline\",\"value\":9.5}. "
            "If user asks about dynamics/trend/chart (оцени динамику, как мой график): "
            "{\"intent\":\"dynamics\"}. "
...
async def _send_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await remember(user_id, "pet", text)
    except Exception as e:
...
async def _process_reading(
    ...
    # Log reading as an event in conversation memory
    event_text = f"хозяйка прислала замер: {value:.1f} ммоль/л"
    if user_context:
        event_text += f" ({user_context})"
    await remember(user_id, "event", event_text)
    ...
    await remember(user_id, "pet", reaction)
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        "🔢 *Напиши число* — `8.5` или `после обеда 12`\n\n"
        "Я вижу динамику (растёт/падает) и *помню наши разговоры* 💭\n\n"
        "*Команды:*\n"
...
async def daily_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if wants_dynamics:
        ...
        await remember(user_id, "event", "хозяйка прислала график динамики")
        await remember(user_id, "pet", reply)
...
async def cmd_newpet(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ctx.user_data.clear()
    # Clear conversation memory — new pet starts fresh
    async with aiosqlite.connect(DB_PATH) as db:
...
async def unknown_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Chat — with conversation memory
        ...
        try:
            memory = await recall(user_id)
            messages = memory_to_messages(memory, text)
            ...
            last_reading = ctx.user_data.get("last_reading")
            reading_note = (
                f"Her most recent glucose reading: {last_reading:.1f} mmol/L. "
                if last_reading else ""
            )
            ...
            chat_msg = await claude.messages.create(
                ...
                system=(
                    ...
                    f"{baseline_note}{reading_note}"
                    "The conversation includes your shared history — remember what she told you before "
                    "and refer back to it naturally when relevant (names, events, plans she mentioned). "
                    "Lines in [brackets] are events (glucose readings she sent). "
            ...
            logger.error(f"Chat error: {e!r}", exc_info=True)
